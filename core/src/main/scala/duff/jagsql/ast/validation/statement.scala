...
// if a from clause is not present, consider we are selecting from DUAL
def addEmptyDual(s: cst.Statement.SelectStatement[Indexed]): cst.Statement.SelectStatement[Indexed] =
  s match {
            ...
            Indexed(
              cst.FromClause(
                NonEmptyList
                  .one(
                    Indexed(
                      cst.FromSource(Indexed(cst.Source.Dual(Indexed("in", Position.empty)), Position.empty), None),
                      Position.empty
                    )
                  )
              ),
...
def analyzeStatement(
  ...
  def analyzeSource(f: cst.Source[Indexed]): Verified[ast.Source] =
    f match {
      ...
      case cst.Source.SubQuery(Indexed(statement, _), Indexed(alias, _)) =>
        for {
          before <- Verified.read
          s      <- analyzeStatement(statement)
          _      <- Verified.set(before + (alias -> s.tableType))
  ...
  def validateFunction(source: cst.Source.TableFunction[Indexed]): Verified[Source] = {
    val singleColString: ComplexType.Table = ComplexType.Table(NonEmptyMap.of("col_0" -> SimpleType.String))

    source match {
      case cst.Source.TableFunction(Indexed("FILE", _), args, Indexed(alias, _))
          if args.map(x => getLiteralType(x.value)) == List(Type.String) =>
        Source.TableFunction("FILE", args.map(_.value), singleColString).asRight.liftTo[Verified]
      case cst.Source.TableFunction(Indexed("RUN", _), args, Indexed(alias, _))
          if args.map(x => getLiteralType(x.value)) == List(Type.String) =>
        Source.TableFunction("RUN", args.map(_.value), singleColString).asRight.liftTo[Verified]
      case _ =>
    ...
    if (groupByExpressions.contains(e.fix)) {
      ...
      e.unlabel match {
        ...
        case LiteralExpression(literal, t)                            =>
          Right(Nil)
        case Unary(e, operator, t)                                    =>
          isValidWithRespectToGroupBy(e, groupByExpressions).as(Nil)
        case _                                                        =>
      ...
      for {
        ...
        types = analyzedProjections
                  ...
                  .map { z =>
                    ...
                    val expressionType: SimpleType = e.unfix.expressionType match {
                      case t: SimpleType => t
                      case t             => sys.error(s"Expressions should return only simple types (here $t)")
                    }
