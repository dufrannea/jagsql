...
import duff.jagsql.parser.{selectStatement, Indexed}
...
class StatementAnalysisSpec extends StatementAnalysisDsl:
  ...
  "SELECT 1" analyzesTo {
    Statement.SelectStatement(
      NonEmptyList.one(Projection(one, None)),
      FromClause(NonEmptyList.of(FromSource(Source.Dual("in"), None))),
      None,
      None,
      ss
    )
  ...
  "Nested scopes" - {
    ...
    """SELECT inaccessible.col_0 FROM (SELECT 1 AS col_0 FROM (SELECT 1 AS col_0) AS inaccessible) AS foo""" failsWith {
      """ERROR: Identifier not found inaccessible
        |
        |SELECT inaccessible.col_0 FROM (SELECT 1 AS col_0 FROM (SELECT 1 AS col_0) AS inaccessible) AS foo
        |       ^^^^^^^^^^^^^^^^^^""".stripMargin
    }
  }
  ...
  "Non fully qualified identifiers resolution" - {
    "SELECT bar FROM (SELECT 1 AS bar) AS foo" succeeds

    "SELECT bar FROM (SELECT 1 AS baz) AS foo JOIN (SELECT 2 AS bar) AS bar ON 1 = 1" succeeds

    "SELECT bar FROM (SELECT 1 AS bar) AS foo JOIN (SELECT 2 AS bar) AS bar ON 1 = 1" failsWith {
      """ERROR: Ambiguous identifier bar
        |
        |SELECT bar FROM (SELECT 1 AS bar) AS foo JOIN (SELECT 2 AS bar) AS bar ON 1 = 1
        |       ^^^""".stripMargin
    }
